...
via the :code:`pytest_configure` and :code:`pytest_unconfigure` hooks:
...
To modify the *Environment* section **after** tests are run, use :code:`pytest_unconfigure`:
...
is important, as this ensures your `pytest_sessionfinish` is run **before** any other plugins ( including `pytest-html` and `pytest-metadata` ) are run.
If this line is omitted, then the *Environment* table will **not** be updated since the `pytest_sessionfinish` of the plugins will execute first, and thus
